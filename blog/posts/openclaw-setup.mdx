...
默认情况下 OpenClaw 会在 `~/.openclaw/workspace` 目录下自动初始化 Git 仓库，但是我不满足于此，懒狗精神发作，希望把整个仓库的重要内容都备份上传到自己的 Github 私有仓库里。

所以我在 `~/.openclaw` 目录下直接初始化了新的 Git 仓库：
...
rm -rf ~/.openclaw/workspace/.git # 删除 OpenClaw 自动初始化的 Git 仓库，避免冲突
```
...
手动操作自然不赖，但是把备份能力自动化更保安心。于是让 AI 帮我编写了一款小脚本 `~/.openclaw/scripts/backup-workspace.sh`：
...
对于 MacOS 系统，可以把这个脚本设置为自启服务，每天定时执行。编写系统服务文件 `~/Library/LaunchAgents/ai.openclaw.backup.plist`：
...
<dict>
    ...
    <!-- 每天北京时间 24:00 执行 -->
    <!-- macOS 使用 UTC，北京时间 24:00 = UTC 16:00 -->
...
现在系统就会自动在每天北京时间晚上十二点，备份 OpenClaw 工作区中我需要的数据与配置文件啦。
