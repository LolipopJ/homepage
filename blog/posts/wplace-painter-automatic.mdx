...
全流程用到了一些**前端调试**的知识，对非技术的小白用户并不友好，请见谅。如果对实现的细节不感兴趣，可以直接跳到最后的[使用方法](#使用方法)部分，按照步骤操作即可。
...
若出现反机器人检测的弹窗，那么按照提示完成验证后，继续执行代码即可。本方案相对其它软件化方案的最大优点也在于此：作为用户的我们始终在真实的浏览器环境操作，所以也能正常地通过反机器人检测。
