...
迷茫之际找到了这个仓库 [`Wplace-Click-Assistant`](https://github.com/Beibeisheep/Wplace-Click-Assistant)，它获取当前浏览器窗口显示的内容，通过计算机视觉技术识别出待点击的区域，并模拟鼠标移动和点击事件来完成像素点放置。即使偶尔会出现精度问题，但从理论上讲已是很完备的方案了，内置的模拟人类操作代码应该也足以应对官方的自动化检测了。稍微遗憾是脚本运行的过程中会持续占用鼠标，这也无可厚非。我并没有试用过此脚本，但放在这里给一个大大的肯定🙆‍♂️。

终于的终于，我还是决定通过**调用接口**的方式来实现自动化放置像素点。经验告诉我，Wplace 肯定在请求里塞了加密 Token，防止大家无脑调用接口，下面就来具体分析看看。
...
于是，从 Blue Marble 获取到待绘制像素点的坐标和颜色信息后，就可以构造出正确的载荷了。为此，我们需要改造一下 Blue Marble 插件。
