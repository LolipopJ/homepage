...
将下载的证书文件传入 Linux 主机中并解压。以 SSL 证书文件压缩包 `blog.towind.cn.zip` 为例：

```bash
# 将文件解压到当前目录下的 blog.towind.cn 目录中
unzip blog.towind.cn.zip -d blog.towind.cn
```

可以将 `blog.towind.cn/Nginx` 目录下的文件放置到 `/www/cert` 目录下：

```bash
mkdir /www/cert
cp blog.towind.cn/Nginx/* /www/cert
```
...
services:
  blog:
    ...
    ports:
      ...
      - 443:443
    environment:
      - NGINX_HOST=blog.towind.cn
      - NGINX_HOST_SSL_CRT=cert/1_blog.towind.cn_bundle.crt
      - NGINX_HOST_SSL_KEY=cert/2_blog.towind.cn.key
```
...
嘿！一切就绪，从浏览器访问我的博客吧！网址是：[https://blog.towind.cn](https://blog.towind.cn/)
