...
验证 Grafana 服务是否启动成功，可以访问 Grafana 容器映射在机器上的端口（配置文件里默认为 `3000`）。如果成功的话，将正常显示控制台页面如下：
...
docker run --name=alist \
        ...
        --volume /path/to/AList/data:/opt/alist/data \
        --network=bridge \
        --workdir=/opt/alist/ \
        -p 5244:5244 \
        --expose=5245 \
...
摘取其中笔者确实使用到的参数，简洁的启动命令就是：`docker run --name=alist --volume /path/to/AList/data:/opt/alist/data -p 5244:5244 --restart=unless-stopped xhofe/alist`。
