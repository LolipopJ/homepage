...
> [!IMPORTANT]
> Souma 基于最终幻想 14 的解包工具 [Lumina](https://github.com/NotAdam/Lumina) 制作了提取文本 EXD 资源的工具 [XivExdUnpacker](https://github.com/Souma-Sumire/XivExdUnpacker)，并且将提取到的文本资源上传到了仓库 [ffxiv-datamining-hexcode-mixed](https://github.com/Souma-Sumire/ffxiv-datamining-hexcode-mixed) 里，供大家下载使用。如果你想跳过手动提取文本资源的步骤，可以直接下载这个仓库里的 CSV 文件来进行后续的学习研究了。

> [!CAUTION]
> Souma 提到工具 SaintCoinach 已不再活跃维护，Gpoint 的改版也有很长时间没有拉取上游更新了，可能会有潜在的兼容性问题。在 SaintCoinach 恢复活跃更新前，可以使用 Souma 开发的 XivExdUnpacker 工具。下面的内容记载于知晓这一事实之前，权作参考吧。
...
> [!NOTE]
> 虽然有着美好的初衷，但 Souma 本人也是一名经验丰富的程序员，我想到的他自然也早就想到了。后来在群里他提到这些 CSV 文件是自动生成的，意即他也应该用到了什么脚本来实现后文中与我相似的处理，只是我没有找到或是他没有开源而已。
...
- 前者的类型定义使用使用小写缩写格式，如 `int32` 和 `str`，后者则使用大驼峰完整格式，如 `Int32` 和 `String`。后者是 Souma 开发的工具 XivExdUnpacker 在导出时采用的命名格式，如果使用此工具导出，或是使用了仓库里现成的文本资源，则无需做此处的替换。
...
这是由于对应的 [`Adventure.csv`](https://github.com/Souma-Sumire/FFXIVChnTextPatch-Souma/blob/996bcf6f2047be5aacfcfd4adc5f1511c4876498/resource/rawexd/Adventure.csv) 文件里没有“风景印象”所对应的第 14 列表格字段。

自动化脚本在处理后会正确包含所有的字符串类型表格列：
...
当然，这并非维护者的疏忽，反而是辛勤劳动的体现。Souma 提到，在版本不同步的时候，国际服某些 CSV 文件里的索引与国服的索引不一致，需要针对文件手动维护一份索引数组，如果某个版本开发者修改了索引，对应的汉化就可能出现问题。不过现如今版本同步了，索引应该也保持一致了，以后应该就不存在必须手动维护索引数组的问题了。
...
已知“替换字体”和“替换文本”功能实际会将字体和文本资源追加到对应数据文件的末尾，游戏更新时通常不会覆写与字体相关的数据文件，所以考虑将两个功能解耦开，不再通过设置里的选项来决定汉化时是否执行替换字体资源的这一耗时操作。

在 Souma 维护的基础上，我修改了相关的逻辑代码，让“替换字体”和“还原字体”作为主界面的功能呈现：

![汉化器主界面](./ffxiv-chn-text-patch-note/汉化器主界面.png)

可以在[此仓库](https://github.com/LolipopJ/FFXIVChnTextPatch-GP)下载编译好的 [`FFXIVChnTextPatchGP.exe`](https://github.com/LolipopJ/FFXIVChnTextPatch-GP/raw/refs/heads/master/FFXIVChnTextPatchGP.exe) 汉化器文件，拖入到 FFXIVChnTextPatch-Souma 根目录（即 `FFXIVChnTextPatch-SM.exe` 同级目录）下使用。
...
还要感谢现任 FFXIVChnTextPatch-Souma 繁中版汉化负责人 LongSaint 的帮助，提供了汉化器项目的现状信息。
