...
date: 2024-05-13
updated: 2026-09-04
banner: ./nginx-services/banner.png
timeliness: -1
categories:
  - 技术琐事
tags:
...
http {
  server {
    ...
    ssl_certificate /etc/letsencrypt/live/towind.cn/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/towind.cn/privkey.pem;

    server_name towind.cn www.towind.cn blog.towind.cn;
    root /var/www/towind.cn/blog;
  }
}
```

通过上面的配置，笔者只需要将自己博客的静态资源放置在 `/var/www/towind.cn/blog` 目录，就可以通过 https://towind.cn 等域名访问到啦。
...
http {
  server {
    ...
    server_name towind.cn www.towind.cn;
    rewrite ^/(.*)$ https://blog.towind.cn/$1 permanent;
  }
}
```

现在，访问 https://towind.cn 和 https://www.towind.cn 时，浏览器将自动 301 重定向到 https://blog.towind.cn。
...
配置 Nginx 启用 `Cache-Control` 缓存，由客户端控制是否使用本地已缓存的文件。配置内容如下，可以根据需要调整：
...
location ~* \.(png|jpg|jpeg|gif|ico|svg|webp|avif|bmp|tiff|tif|mp4|webm|ogg|ogv|mp3|wav|flac|aac|mov|avi|mkv|htc|woff|woff2|ttf|eot|otf)$ {
    add_header Cache-Control "public, max-age=31536000, immutable" always;
    expires 1y;
}

location ~* \.(json|xml|rss|atom|txt|csv|yaml|yml|md)$ {
    add_header Cache-Control "private, no-cache, must-revalidate" always;
    expires 0;
}
```
...
server {
  ...
  server_name download.towind.cn;
}

server {
  include /etc/nginx/conf.shared.d/https.conf;
  server_name download.towind.cn;
  root /var/www/towind.cn/download;
  # 允许浏览静态文件目录
  autoindex on;
  # 显示静态文件大小
...
上面的配置表明：笔者将对外提供下载的静态文件放置在服务器的 `/var/www/towind.cn/download` 目录下；当访问 https://download.towind.cn 时，就可以看见所有可下载的静态文件。
...
上面的配置表示：当通过 `https://xxx.towind.cn` 访问公网服务器时，请求将转发至本地的 15244 端口，再经由 frp 访问到内网服务器上对应的服务。
