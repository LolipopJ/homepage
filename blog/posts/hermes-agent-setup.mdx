...
撰写此文时，Hermes Agent 的版本号为 v0.8.0，翻看终端日志可知，一键脚本在 MacOS 系统上依次安装了：
...
接下来，基于 Docker 部署 Camofox Browser 服务。撰写此章节时 Camofox Browser 的版本号为 v1.5.2，由于官方并没有提供上传到 Docker Hub 的镜像，所以需要我们克隆原仓库并手动构建镜像：
...
第一步，修改 Hermes Agent 的配置文件 `~/.hermes/config.yaml`，启用持久 Camofox Browser 会话的能力：
...
启动后，对于相同的 Hermes Agent 档案，在调用 Camofox Browser 时，总会使用形如 `hermes_xxxxxxxxxx` 的稳定 `userId`，这样，当之后为该 ID 导入 Cookies 数据后，助理就能以我的身份进行各种操作了。从上一小节最后的日志里也可以看到，我电脑上的 Hermes Agent 的 `userId` 是 `hermes_deef0197d7`。

第二步，生成一个专用于调用 Camofox Browser 服务接口的密钥：
...
第三步，将刚刚生成的密钥添加到 Camofox Browser 服务的环境变量中：
...
第四步，将密钥也添加到 Hermes Agent 的环境变量中：

```bash
CAMOFOX_API_KEY="your-generated-key"
```

未来如果要让助理代劳导入 Cookies 数据，它就可以在请求头里添加这个密钥了。

第五步，在工作电脑的浏览器上安装可以导出站点 Cookies 为 Netscape 格式文件的拓展程序，如 Chrome 浏览器的拓展程序 [Get cookies.txt LOCALLY](https://chromewebstore.google.com/detail/get-cookiestxt-locally/cclelndahbckbenkjhflpdbgdldlbecc)。
...
第六步，让 Hermes Agent 帮我把 Cookies 数据导入到 Camofox Browser 服务中。
...
大功告成！下面开始对 Camofox Browser 的反检测能力进行测试。
...
喔对了，因为目前版本的 Camofox Browser 还不支持持久化 Cookies 等数据（官方文档里写的环境变量 `CAMOFOX_PROFILE_DIR` 还没有实现，可能是 AI 生成的幻觉；另外让 AI 分析 Camofox Browser 源码，也未发现相关实现），每次通过 `curl` 请求导入的数据有效期为当前会话，而会话过期的时间默认为 10 分钟。如果会话过期了，Cookies 数据也就清空了，就会出现上面未登录的提示信息。

那么，再执行一遍 `curl -X POST http://localhost:9377/sessions/agent1/cookies` 请求，接着让助理重新尝试吧：
...
它做到了！Camofox Browser 成功绕过了 V2EX 的自动化检测机制，让助理顺利完成了签到任务！太棒了！
...
examples:
  - description: V2EX sign-in task
    code: |
      ...
      curl -X POST http://localhost:9377/sessions/${SESSION_ID}/cookies \
        -H 'Content-Type: application/json' \
        -H "Authorization: Bearer ${CAMOFOX_API_KEY}" \
        -d "$COOKIES"
...
environment_variables:
  ...
  - SESSION_ID: Current Hermes browser session identifier
...
接下来，只要不会遇到 Cloudflare 之类的拦截，想必什么事情都能让 Hermes Agent 代劳了吧！
