  ...
  function (e) {
    for (;;)
      switch ((e.prev = e.next)) {
        ...
        case 33:
          if (((S = e.sent), (k = S.data), (w = k.nonce))) {
            // w 为调用 /sign 接口时随机生成的参数 ${nonce}
            e.next = 38;
            break;
          }
...
可知，笔者所需的真正密钥为 `a1c18f6619b84605`。

## 下载并拼装视频文件

剩下的工作就非常简单了，将真正密钥保存为本地的文件 `key.bin`，并且修改 `playlist.m3u8` 里 `#EXT-X-KEY` 行的 `URI` 字段为本地文件路径：
