...
经过调研，笔者找到了 [`discord.py-self`](https://github.com/dolfies/discord.py-self) 库，考虑通过 Discord Self-Bot 来实现相关功能。**当然，直接轮询调用 Discord 官方提供的 API 已然可以实现笔者的需求，只是笔者太闲了想在外面包一层 Self-Bot**……😎万一以后用得上呢？

> Discord 官方文档中明确表示**不支持 Self-Bot 的使用**，违反使用条款可能导致账号被封禁。因此，笔者仅将本文内容作为技术分享，若您决心实现类似功能，请承担账号封禁的风险。

整个步骤简单来说就是：创建一个 Bot 子账号，添加笔者主账号好友，让 Bot 轮询笔者主账号的状态信息，将其提供给前端访问。即使真的触到霉头被官方封禁的话，去世的也只是笔者的子账号……吧。
...
笔者翻阅 `discord.py-self` 的文档良久，终于找到了 `get_relationship(user_id)` 方法，可以获取当前登录用户（子账号）与指定用户（主账号）的好友关系。好友关系如何并不重要，重要的是这个接口的返回值里囊括了指定用户的状态信息。在上面的代码中，笔者让服务每五秒执行此方法并得到最新的状态信息；环境变量 `DISCORD_CHANNEL_ID` 是 Bot 与笔者主账号的私聊频道 ID，同样可以通过浏览器开发者工具找到网络请求找到。
