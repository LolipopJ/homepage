...
- 2025.05.13（v1.4），修正部分措辞，更新服务器人口统计数据。
...
下载最新版本的[汉化文件压缩包](https://github.com/Souma-Sumire/FFXIVChnTextPatch-Souma/releases)（命名形如 `7.21.v1.zip`），将解压后得到的文件覆盖到 `FINAL FANTASY XIV - A Realm Reborn\game\sqpack\ffxiv` 目录。

### 插件加载方案

拓展性更好的汉化方案是通过卫月加载汉化模组，但是需要一定的动手能力。此外由于每次客户端版本更新时，卫月会无法使用（避免旧版本框架导致的游戏崩溃；根据版本更新的大小，卫月更新所需的时间也从一天到几天不等），汉化模组也**暂时无法加载**，需要忍受未汉化的客户端。
...
过去笔者用过的汉化方案是：基于开源的 [FFXIVChnTextPatch-GP](https://github.com/GpointChen/FFXIVChnTextPatch-GP) 工具来汉化游戏客户端，详细步骤请参考其文档。一个比较大的缺陷是[物品搜索](#物品搜索)等地方需要用游戏客户端原文，否则无法正常使用。
...
- **Elemental** 有着来自世界各地的玩家，可以直接使用英语交流。
- **Gaia** 日本玩家居多，也有不少的台湾、香港玩家。
- **Mana** 人数最多的数据中心，日本玩家居多，同时聚集着大量的国人玩家。
- **Meteor** 很少去，不清楚。
...
使用 ACT 是一个两端都很锋利的双刃剑：好的方面是会让你认真对待每一场战斗，逐渐优化自己的输出循环，拼尽全力做到力所能及的最好，为游戏多增添一些可挑战的地方；坏的方面是会降低你对“摸鱼”队友的容忍度，并且会为自己比别人高的数据感到傲慢，比别人低的数据感到焦虑。
